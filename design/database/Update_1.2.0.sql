...
INSERT INTO SYSTEM_PARAM(ID,TYPE,VALUE,MEMO) VALUES('IMG_SERVER_ADDRESS','STRING','http://192.168.0.8:8090/','图片服务器地址');
INSERT INTO SYSTEM_PARAM(ID,TYPE,VALUE,MEMO) VALUES('FILE_SERVER_ADDRESS','STRING','http://192.168.0.8:9333/','文件服务器地址');
...
UPDATE  HOUSE_OWNER_RECORD.UPLOAD_FILE set UPLOAD_TIME = '2015-5-17' where UPLOAD_TIME = 0;
...
INSERT INTO ROLE_FUNCTION (ROL_ID, FUN_ID) VALUES ('recordRunManager','owner.recordStore');

INSERT INTO ROLE_FUNCTION (ROL_ID, FUN_ID) VALUES ('recordRunManager','owner.recordRoom');

UPDATE HOUSE_OWNER_RECORD.RECORD_STORE set VERSION = 1 where VERSION is NULL;
UPDATE HOUSE_OWNER_RECORD.RECORD_LOCAL set VERSION = 1 WHERE VERSION is NULL;
...
INSERT INTO DB_PLAT_SYSTEM.SYSTEM_PARAM(ID,TYPE,VALUE,MEMO) VALUES('ShouldMoneyAllowChange','BOOLEAN','false','应收可是否可由用户修改');
...
INSERT DB_PLAT_SYSTEM.SYSTEM_PARAM(ID, TYPE, VALUE, MEMO) VALUES ('PatchFileUpload','BOOLEAN','false','档案补录是否上传文件');
...
INSERT INTO HOUSE_OWNER_RECORD.HOUSE_AND_RECORD(HOUSE,RECORD)  select h.ID,rs.ID from BUSINESS_HOUSE h LEFT JOIN OWNER_BUSINESS ob on ob.ID = h.BUSINESS_ID
 LEFT JOIN RECORD_STORE rs on rs.BUSINESS = h.BUSINESS_ID where
 ob.STATUS in ('COMPLETE' , 'COMPLETE_CANCEL', 'MODIFYING')
 and ob.SOURCE ='';
...
ALTER TABLE HOUSE_OWNER_RECORD.LAND_INFO MODIFY BEGIN_USE_TIME DATETIME;
