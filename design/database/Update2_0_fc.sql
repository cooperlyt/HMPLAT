...
INSERT INTO HOUSE_OWNER_RECORD.POWER_OWNER(ID,NAME,ID_TYPE,ID_NO,PHONE,LEGAL_PERSON,LEGAL_TYPE,ROOT_ADDRESS,ADDRESS,TYPE,PRI,CARD,OLD)
  SELECT h.ID,bo.NAME,bo.ID_TYPE,bo.ID_NO,bo.PHONE,bo.LEGAL_PERSON,'LEGAL_OWNER',bo.ADDRESS,bo.ROOT_ADDRESS,'OWNER',0,bo.OWNER_CARD,false FROM HOUSE_OWNER_RECORD.BUSINESS_HOUSE bh LEFT JOIN HOUSE h on h.ID = bh.AFTER_HOUSE LEFT JOIN BUSINESS_OWNER bo on bo.ID = h.MAIN_OWNER where not h.MAIN_OWNER is null;

INSERT INTO HOUSE_OWNER(HOUSE,POOL)
  SELECT h.ID,h.ID FROM HOUSE_OWNER_RECORD.BUSINESS_HOUSE bh LEFT JOIN HOUSE h on h.ID = bh.AFTER_HOUSE LEFT JOIN BUSINESS_OWNER bo on bo.ID = h.MAIN_OWNER where not h.MAIN_OWNER is null;



INSERT INTO HOUSE_OWNER_RECORD.POWER_OWNER(ID,NAME,ID_TYPE,ID_NO,PHONE,LEGAL_PERSON,LEGAL_TYPE,ROOT_ADDRESS,ADDRESS,TYPE,PRI,CARD,OLD)
  SELECT bh.ID,bo.NAME,bo.ID_TYPE,bo.ID_NO,bo.PHONE,bo.LEGAL_PERSON,'LEGAL_OWNER',bo.ADDRESS,bo.ROOT_ADDRESS,'INIT',0,bo.OWNER_CARD,false FROM HOUSE_OWNER_RECORD.BUSINESS_HOUSE bh LEFT JOIN HOUSE h on h.ID = bh.AFTER_HOUSE LEFT JOIN BUSINESS_OWNER bo on bo.ID = h.OLD_OWNER where not h.OLD_OWNER is null;

INSERT INTO HOUSE_OWNER(HOUSE,POOL)
  SELECT h.ID,bh.ID FROM HOUSE_OWNER_RECORD.BUSINESS_HOUSE bh LEFT JOIN HOUSE h on h.ID = bh.AFTER_HOUSE LEFT JOIN BUSINESS_OWNER bo on bo.ID = h.OLD_OWNER where not h.OLD_OWNER is null;
...
INSERT INTO HOUSE_OWNER_RECORD.POWER_OWNER(ID,NAME,ID_TYPE,ID_NO,PHONE,LEGAL_PERSON,LEGAL_TYPE,ROOT_ADDRESS,ADDRESS,TYPE,PRI,OLD)
  SELECT concat(RIGHT(co.BUSINESS,10),RIGHT(h.ID,10),RIGHT(co.ID,12)),co.NAME,co.ID_TYPE,co.ID_NO,co.PHONE,co.LEGAL_PERSON,'LEGAL_OWNER',co.ROOT_ADDRESS,co.ADDRESS,'CONTRACT',0,FALSE FROM CONTRACT_OWNER co LEFT JOIN HOUSE h on h.CONTRACT_OWNER = co.ID where not h.ID is null;


INSERT INTO HOUSE_OWNER(HOUSE,POOL)
  SELECT h.ID,concat(RIGHT(co.BUSINESS,10),RIGHT(h.ID,10),RIGHT(co.ID,12)) FROM CONTRACT_OWNER co LEFT JOIN HOUSE h on h.CONTRACT_OWNER = co.ID where not h.ID is null;
...
INSERT INTO HOUSE_CONTRACT(ID,CONTRACT_NUMBER,CONTRACT_DATE,TYPE,PROJECT_RSHIP_NUMBER,BUSINESS) select co.ID,co.CONTRACT_NUMBER,co.CONTRACT_DATE,co.TYPE,co.PROJECT_RSHIP_NUMBER,co.BUSINESS from CONTRACT_OWNER co where not (co.ID in (select ID FROM HOUSE_CONTRACT));
...
-- 因为合并了 USE_TYPE 所以有重的USE_TYPE 暂时删除约束
ALTER TABLE HOUSE_OWNER_RECORD.LAND_END_TIME DROP FOREIGN KEY LAND_END_TIME_ibfk_1;
DROP INDEX LAND_TIME_PROJECT_USE_TYPE ON HOUSE_OWNER_RECORD.LAND_END_TIME;

-- Duplicate entry 'ff80808152aa6c3201531b6570d108ae-1' for key 'LAND_TIME_PROJECT_USE_TYPE'
...
-- 以下根据地区修改
UPDATE HOUSE_OWNER_RECORD.HOUSE set HOUSE_TYPE = 'SALE_HOUSE' where HOUSE_TYPE is null;
UPDATE HOUSE_OWNER_RECORD.HOUSE set HOUSE_TYPE = 'BACK_HOUSE' where HOUSE_TYPE = '1946';
UPDATE HOUSE_OWNER_RECORD.HOUSE set HOUSE_TYPE = 'GOV_GROUP_HOUSE' where HOUSE_TYPE = '206';
UPDATE HOUSE_OWNER_RECORD.HOUSE set HOUSE_TYPE = 'OTHER' where HOUSE_TYPE = '4104';
...
UPDATE HOUSE_INFO.HOUSE set HOUSE_TYPE = 'SALE_HOUSE' where HOUSE_TYPE is null;
UPDATE HOUSE_INFO.HOUSE set HOUSE_TYPE = 'BACK_HOUSE' where HOUSE_TYPE = '1946';
UPDATE HOUSE_INFO.HOUSE set HOUSE_TYPE = 'GOV_GROUP_HOUSE' where HOUSE_TYPE = '206';
UPDATE HOUSE_INFO.HOUSE set HOUSE_TYPE = 'OTHER' where HOUSE_TYPE = '4104';
...
UPDATE HOUSE_INFO.GRID_BLOCK set HOUSE_TYPE = 'SALE_HOUSE' where HOUSE_TYPE is null;
UPDATE HOUSE_INFO.GRID_BLOCK set HOUSE_TYPE = 'BACK_HOUSE' where HOUSE_TYPE = '1946';
UPDATE HOUSE_INFO.GRID_BLOCK set HOUSE_TYPE = 'GOV_GROUP_HOUSE' where HOUSE_TYPE = '206';
UPDATE HOUSE_INFO.GRID_BLOCK set HOUSE_TYPE = 'OTHER' where HOUSE_TYPE = '4104';
...
INSERT INTO HOUSE_OWNER_RECORD.SELL_TYPE_TOTAL(ID, BUILD_ID, USE_TYPE, COUNT, AREA)
  SELECT concat(RIGHT(ID,30),'-H'),ID,'DWELLING_KEY',HOME_COUNT,if(HOME_AREA is null , 0, HOME_AREA) FROM HOUSE_OWNER_RECORD.BUILD;

INSERT INTO HOUSE_OWNER_RECORD.SELL_TYPE_TOTAL(ID, BUILD_ID, USE_TYPE, COUNT, AREA)
  SELECT concat(RIGHT(ID,30),'-S'),ID,'SHOP_HOUSE_KEY',SHOP_COUNT,if(SHOP_AREA is null , 0, SHOP_AREA)  FROM HOUSE_OWNER_RECORD.BUILD;

INSERT INTO HOUSE_OWNER_RECORD.SELL_TYPE_TOTAL(ID, BUILD_ID, USE_TYPE, COUNT, AREA)
  SELECT concat(RIGHT(ID,30),'-U'),ID,'OTHER',UNHOME_COUNT,if(UNHOME_AREA is null , 0, UNHOME_AREA) FROM HOUSE_OWNER_RECORD.BUILD;
...
INSERT INTO DB_PLAT_SYSTEM.ROLE (ID, NAME, DESCRIPTION, PRIORITY)  VALUES('total.export.saleTotal','房地产交易情况表','房地产交易情况表',5);
INSERT INTO DB_PLAT_SYSTEM.ROLE(ID, NAME, DESCRIPTION, PRIORITY) VALUES ('total.export.storeHouse','库存商品房统计','库存商品房统计',8);


ALTER TABLE HOUSE_OWNER_RECORD.BUSINESS_FILE ADD TYPE VARCHAR(20) NOT NULL;

-- ...

UPDATE BUSINESS_FILE set TYPE = 'IMPORTANT' WHERE IMPORTANT = true;

UPDATE BUSINESS_FILE set TYPE = 'ADDITIONAL' WHERE IMPORTANT = false;
ALTER TABLE HOUSE_OWNER_RECORD.BUSINESS_FILE DROP IMPORTANT;



-- 导库需要
ALTER TABLE HOUSE_OWNER_RECORD.OWNER_BUSINESS ADD o_id VARCHAR(32) NULL;
ALTER TABLE HOUSE_OWNER_RECORD.OWNER_BUSINESS ADD o_file VARCHAR(200) NULL;

ALTER TABLE HOUSE_OWNER_RECORD.OWNER_BUSINESS ADD IN_ROOM BOOLEAN NULL;
UPDATE HOUSE_OWNER_RECORD.OWNER_BUSINESS set IN_ROOM = false;


ALTER TABLE HOUSE_OWNER_RECORD.RECORD_STORE ADD CABINET VARCHAR(32)  NULL;
ALTER TABLE HOUSE_OWNER_RECORD.RECORD_STORE ADD FRAME VARCHAR(32)  NULL;
ALTER TABLE HOUSE_OWNER_RECORD.RECORD_STORE ADD BOX VARCHAR(32)  NULL;
ALTER TABLE HOUSE_OWNER_RECORD.RECORD_STORE ADD ROOM VARCHAR(32)  NULL;


UPDATE DB_PLAT_SYSTEM.SYSTEM_PARAM set VALUE='2.0.1' where ID='database_version';
...
4.  连接上下手， 查HouseCode , 计算 mainOwner, 检查 keep house_CODE
