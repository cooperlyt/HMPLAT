...
public class NewHouseRecordTotalDataProvider implements RestDataProvider{
    ...
    public JSONObject getJsonData() {
        ...
        HouseSaleTotalData home =  ownerEntityLoader.getEntityManager().createQuery("select new com.dgsoft.house.owner.total.data.HouseSaleTotalData(sum(bb.homeCount),sum(bb.homeArea)) from BusinessBuild bb where bb.businessProject.ownerBusiness.status in ('MODIFYING','COMPLETE')", HouseSaleTotalData.class).getSingleResult();

        HouseSaleTotalData sale = ownerEntityLoader.getEntityManager().createQuery("select new com.dgsoft.house.owner.total.data.HouseSaleTotalData(count(hb.id),sum(hb.afterBusinessHouse.houseArea),sum(hb.afterBusinessHouse.saleInfo.sumPrice) ) from HouseBusiness hb where hb.ownerBusiness.defineId =:newRecordBizId and hb.ownerBusiness.status in ('RUNNING', 'COMPLETE', 'SUSPEND', 'MODIFYING')", HouseSaleTotalData.class).setParameter("newRecordBizId", RunParam.instance().getStringParamValue("NewHouseContractBizId")).getSingleResult();

        HouseSaleTotalData saleHome = ownerEntityLoader.getEntityManager().createQuery("select new com.dgsoft.house.owner.total.data.HouseSaleTotalData(count(hb.id),sum(hb.afterBusinessHouse.houseArea),sum(hb.afterBusinessHouse.saleInfo.sumPrice) ) from HouseBusiness hb where hb.afterBusinessHouse.useType in (:homeType) and hb.ownerBusiness.defineId =:newRecordBizId and hb.ownerBusiness.status in ('RUNNING', 'COMPLETE', 'SUSPEND', 'MODIFYING')", HouseSaleTotalData.class).setParameter("newRecordBizId", RunParam.instance().getStringParamValue("NewHouseContractBizId")).setParameter("homeType", UseTypeWordAdapter.instance().getDwellingTypes()).getSingleResult();
