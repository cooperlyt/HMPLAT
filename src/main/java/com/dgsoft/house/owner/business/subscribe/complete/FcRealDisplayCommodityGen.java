 ...
 * 凤城商品房查询显示
 */
@Name("fcRealDisplayCommodityGen")
public class FcRealDisplayCommodityGen implements TaskCompleteSubscribeComponent {
    ...
    public void complete() {
        for(HouseBusiness bh: ownerBusinessHome.getInstance().getHouseBusinesses()){
            ...
            businessDisplay.addData(DescriptionDisplay.DisplayStyle.LABEL, "开发商名称 ");
            String ycontractPersonNames = "";
            for (PowerPerson pp: bh.getAfterBusinessHouse().getPowerPersonListByType(PowerPerson.PowerPersonType.INIT,true)){
                if (pp.getType().equals(PowerPerson.PowerPersonType.INIT)){
