 ...
 * 转移预告抵押
 */
@Name("makeCardNoticeMortgageDivert")
public class MakeCardNoticeMortgageDivert implements TaskCompleteSubscribeComponent {
    ...
    public void complete() {
        for (MakeCard m:ownerBusinessHome.getInstance().getMakeCards()){
            if(m.getType().equals(MakeCard.CardType.NOTICE_DIVERT_MORTGAGE)){
                return;
            }
        }
        ...
        String no;
        if (typeCard==2){
            no= datePart + Long.toString(numberBuilder.getNumber(MakeCard.CardType.NOTICE_DIVERT_MORTGAGE.name()));
        ...
        MakeCard makeCard = new MakeCard(MakeCard.CardType.NOTICE_DIVERT_MORTGAGE, no);
