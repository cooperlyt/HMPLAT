        ...
        if (isCodeDefined()) {
            for (LockedHouse lh : getLockedHouses()) {
                if (LockType.HOUSE_LOCKED.equals(lh.getType())) {
                    ownerEntityLoader.getEntityManager().persist(new LockedHouseCancel(lh.getHouseCode(),authInfo.getLoginEmployee().getId(),authInfo.getLoginEmployee().getPersonName(),new Date(),lh.getDescription(),lh.getType(),lh.getEmpCode(),lh.getEmpName()));
